...
def parse_args():
    # 在 PyTorch 中加载 CvT-13 模型并保存为 .pt 文件，如已准备 ImageNet 数据集，可选择测试网络精度 ------------------------
...
def main():
    ...
    # if prepare ImageNet dataset, then load data
    if args.prepare_data != 0:
    ...
    # if prepare ImageNet dataset, then do the test
    if args.prepare_data != 0:
    ...
    model.eval()
    torch.save(model, "./CvT-13.pt")
    logging.info('=> Succeeded building model in PyTorch!')
    ...
    logging.info('=> Succeeded converting model into onnx!')
